...
const byEdge = (a: ConflictEdge, b: ConflictEdge): number =>
  a.a - b.a || a.b - b.b;
...
export function buildPlan(input: PlanningInput): Plan {
  ...
  const ready = planBatches(
    eligible,
    conflicts.filter((edge) =>
      eligibleNumbers.has(edge.a) && eligibleNumbers.has(edge.b)
    ),
    ordering.filter((edge) =>
      eligibleNumbers.has(edge.before) && eligibleNumbers.has(edge.after)
    ),
  );
...
function findOverlaps(nodes: readonly PrNode[]): ConflictEdge[] {
  ...
  for (let leftIndex = 0; leftIndex < nodes.length; leftIndex += 1) {
    ...
    for (
      ...
      if (paths.length > 0) {
        result.push({ a: left.number, b: right.number, paths });
      }
    }
  }
  return result;
...
function planBatches(
  ...
  const nodeByNumber = new Map(
    nodes.map((node) => [node.number, node] as const),
  );
  ...
  while (remaining.size > 0) {
    const ready = [...remaining].filter((number) =>
      isSubset(required(predecessors, number), placed)
    );
    if (ready.length === 0) {
    ...
    ready.sort((left, right) => {
      ...
      return (
        ...
        leftNode.additions + leftNode.deletions -
          (rightNode.additions + rightNode.deletions) ||
    ...
    for (const number of ready) {
      if (batch.every((peer) => !required(conflicts, number).has(peer))) {
        batch.push(number);
      }
    }
  ...
  const rebaseOrder = [...numbers].sort(
    (left, right) =>
      (batchOf.get(left) ?? 0) - (batchOf.get(right) ?? 0) || left - right,
  );
  for (const number of rebaseOrder) {
    ...
    const earlierDependencies = [...required(predecessors, number)]
      .filter((peer) => (batchOf.get(peer) ?? 0) < currentBatch)
      .sort((a, b) => a - b);
    const after = [...new Set([...earlierConflicts, ...earlierDependencies])]
      .sort((a, b) => a - b);
...
function findHeld(
  nodes: readonly PrNode[],
  ordering: readonly OrderingEdge[],
): HeldPr[] {
  const held = new Map<PrNumber, string[]>();
  for (const node of nodes) {
    const reasons: string[] = [];
    if (node.draft) reasons.push("draft");
    if (node.base_conflict_paths.length > 0) {
      reasons.push("local-base-conflict");
    }
    if (node.mergeable === "CONFLICTING") {
      reasons.push("github-base-conflicting");
    }
...
function setMap(numbers: ReadonlySet<PrNumber>): Map<PrNumber, Set<PrNumber>> {
  return new Map(
    [...numbers].map((
      number,
    ): [PrNumber, Set<PrNumber>] => [number, new Set()]),
  );
}
